...
3. **Channel Name Hook** (`mt_protocol.cpp:handle_channel_tag`): Adds `handleChannelResponse(channel)` call so channel names received during the GCM boot config dump (and any subsequent `get_channel_request` responses) are forwarded to `src/` for display on the Canned Messages screen channel selector button. Declaration is in `lib/meshtastic_customizations/config_callback.h`; implementation in `src/communication/meshtastic_admin.cpp`.
4. **Node Info Hook** (`mt_protocol.cpp:handle_node_info`): Removes the early-exit guard that discarded NodeInfo packets when `node_report_callback` was NULL (i.e., outside an active `get_node_list` request), and adds `handleNodeInfo(nodeNum, shortName)` call so every spontaneous NodeInfo broadcast populates the short-name cache in `src/communication/meshtastic_admin.cpp`. Used by the Canned Messages destination dropdown to show `"SNME !XXXXXXXX"` labels for favorited nodes.
