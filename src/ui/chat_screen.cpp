...
static void newComposeClickedCb(lv_event_t *e) {
    // Tell the canned hub to enter "new compose" mode. The EEZ flow
    // on btn_keyboard handles the screen change to the canned hub.
...
void chatScreenInit() {
    ...
    if (objects.btn_keyboard) {
        lv_obj_add_event_cb(objects.btn_keyboard, newComposeClickedCb,
...
void chatScreenRefresh() {
    ...
    size_t n = chatBufferSnapshot(filter, snapshot, CHAT_BUFFER_SIZE);
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        lv_obj_set_user_data(row, (void *)(uintptr_t)m->id);

        // Reverse-video on outgoing rows is the direction indicator.
        // Incoming rows: black bg, white text. Outgoing: white bg,
        // black text.
        lv_color_t row_bg   = m->outgoing ? lv_color_white() : lv_color_black();
        lv_color_t row_text = m->outgoing ? lv_color_black() : lv_color_white();
        lv_obj_set_style_bg_color(row, row_bg, LV_PART_MAIN);

        lv_obj_t *lbl = lv_label_create(row);
        lv_obj_set_style_text_color(lbl, row_text, LV_PART_MAIN);
        lv_label_set_long_mode(lbl, LV_LABEL_LONG_DOT);
        lv_obj_set_width(lbl, lv_pct(100));

        char rowText[MAX_MESHTASTIC_PAYLOAD + 64];
